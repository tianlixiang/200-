...
@implementation i065ViewController
...
- (UIView *)infoView {
    if (!_infoView) {
        ...
        CXLosedView * view = [[CXLosedView alloc] initWithFrame:CGRectMake(10, self.loseView.frameBottom + 5, KScreenWidth - 20, 131)];
        view.tag = 105;
        [view setName:@"iPhone6 S" price:@"6000" image:@"http://www.photophoto.cn/m15/032/003/0320030231.jpg" time:9000 companyName:@"爱情海购物公司"];
        _infoView = view;
...
- (UIView *)bottomView {
    if (!_bottomView) {
        ...
        [btn1 setBackgroundImage:[UIImage imageNamed:@"icon_fuchou"] forState:UIControlStateNormal];
        btn1.frame = CGRectMake(view.frameRight - 100, 70, 70, 70);
        [btn1 addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
        btn1.tag = 500;
        [view addSubview:btn1];
        
        UIButton * btn2 = [UIButton buttonWithType:UIButtonTypeCustom];
        [btn2 setBackgroundImage:[UIImage imageNamed:@"icon_zaiqiangyici"] forState:UIControlStateNormal];
        btn2.frame = CGRectMake(view.frameRight - 100, btn1.frameBottom + 18, 70, 70);
...
- (void)buttonClick:(UIButton *)button {
    if (button.tag == 500) {
        [self buttonClick:button.tag withTitle:@"找ta复仇"];
    }else {
...
- (void)reloadOtherView {
    CXLosedView * view = (CXLosedView *)[self.view viewWithTag:105];
    [view setName:self.data[@"l_name"] price:self.data[@"l_price"] image:self.data[@"l_image"] time:[self.data[@"l_time"] integerValue] companyName:self.data[@"companyName"]];
}

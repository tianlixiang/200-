...
@interface CX_DeliveryTableViewCell()
{
...
@implementation CX_DeliveryTableViewCell

#pragma mark - **************** 初始化单元格 ****************
+(instancetype)cellWithTableView:(UITableView *)tableView indentifier:(NSString *)indentifier
{
    CX_DeliveryTableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:indentifier];
    if(!cell){
        cell=[[CX_DeliveryTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:indentifier];
...
- (UIView *)bottomView {
    ...
    _payButton.frame = CGRectMake((bottomView.frame.size.width / 3) * 2 - 5, 7, (bottomView.frame.size.width / 3) - 5, (bottomView.frame.size.height - 14));
    
    [_payButton setTitle:@"提醒卖家发货" forState:UIControlStateNormal];
    
    _payButton.titleLabel.font = [UIFont systemFontOfSize:14];
    
    [_payButton setBackgroundColor:[UIColor colorWithHexString:@"0x43b6ff"]];

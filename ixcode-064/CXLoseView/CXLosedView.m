...
@implementation CXLosedView

- (void)setName:(NSString *)name price:(NSString *)price image:(NSString *)imageURL time:(NSInteger )time companyName:(NSString *)companyName {
    self.name = name;
    self.price = price;
...
- (UIView *)rightView {
    ...
    _timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, priceLabel.frameBottom + 10, view.frameWidth, 10)];
    ...
    if (h<10) {
        _timeLabel.text = [NSString stringWithFormat:@"距离兑现还有:0%ld:%ld:%ld",h,f,s];
    }else {
        _timeLabel.text = [NSString stringWithFormat:@"距离兑现还有:%ld:%ld:%ld",h,f,s];
    }
    _timeLabel.font = __font(10);
    [view addSubview:_timeLabel];
    
    _infoLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, _timeLabel.frameBottom + 7, view.frameWidth, 10)];
...
-(void)startTime{
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if(timeout<0){ //倒计时结束，关闭
            ...
            if (h<10) {
                strTime = [NSString stringWithFormat:@"距离兑现还有:0%ld:%ld:%ld",h,f,s];
            }else {
                strTime = [NSString stringWithFormat:@"距离兑现还有:%ld:%ld:%ld",h,f,s];
            }
            dispatch_async(dispatch_get_main_queue(), ^{
                //设置界面的按钮显示 根据自己需求设置
